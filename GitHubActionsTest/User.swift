...
return age>=18
    ...
    func hasRole(_ role :String)->Bool{return roles.contains(role)}

  mutating func deactivate(){isActive=false}
    mutating func addRole(_ role:String){if !roles.contains(role){roles.append(role)}}
}
...
class UserRepository{
  ...
  private let queue=DispatchQueue(label:"com.app.userrepo",attributes:.concurrent)
    ...
    func add(_ user:User){queue.async(flags:.barrier){self.users.append(user)}}

  func find(byId id:Int)->User?{
        return queue.sync{users.first{$0.id==id}}
    }

func find(byEmail email:String)->User?{queue.sync{users.first{$0.email.lowercased()==email.lowercased()}}}
...
return find(byId:user.id) != nil
  ...
  func all()->[User]{return queue.sync{users}}

    func count()->Int{queue.sync{users.count}}
